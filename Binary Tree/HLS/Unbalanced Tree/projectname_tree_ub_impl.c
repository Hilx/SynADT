...
/* Application Level */
/* Create Tree */
ptr_t TreeGen(data_t *Master2Mem, data_t *Master2SysAlloc, ptr_t stackPtr, int NumberOfNodes){
	int i;
	...
	for(i = 0; i < NumberOfNodes; i++){		
		if(i == 0){
			root2return = Insert(Master2Mem, Master2SysAlloc, stackPtr, root2return,inputArray[i]);
		}else{
			insertResult = Insert(Master2Mem, Master2SysAlloc, stackPtr, root2return,inputArray[i]);
		}
	}
	return root2return;
}
