...
ptr_t PM_1_INSERTION(data_t *Master2Mem, data_t *Master2SysAlloc, int log2size, int *buffPtr){
	// create table
	...
	if(TestCase == USE_SEQUENTIAL_KEYS){
		...
		for(i = 0; i < iter; i++){
			int key = 16*(i+1);
			InsertNodeHashTable(Master2Mem, Master2SysAlloc, hdTable, key, buffPtr);
		}
	}else{
...
ptr_t PM_2_CHECK_INSERTION(data_t *Master2Mem, data_t *Master2SysAlloc, ptr_t hdTable, int log2size, int *buffPtr){
	if(TestCase == USE_SEQUENTIAL_KEYS){
		int m = log2size - 1;
		int iter = 1 << m;
		int i;
		for(i = 0; i < iter; i++){
			...
			CheckInsert_HashTable(Master2Mem, Master2SysAlloc, hdTable, key, buffPtr);
		}
	}else{
...
ptr_t PM_3_UPDATE(data_t *Master2Mem, data_t *Master2SysAlloc, ptr_t hdTable, int log2size, int *buffPtr){
	if(TestCase == USE_SEQUENTIAL_KEYS){
		int m = log2size - 1;
		int iter = 1 << m;
		int i;
		for(i = 0; i < iter; i++){
			...
			UpdateNode_HashTable(Master2Mem, Master2SysAlloc, hdTable, oldKey, newKey, buffPtr);
		}
	}else{
