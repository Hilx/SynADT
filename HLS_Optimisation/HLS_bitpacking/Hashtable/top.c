...
int CHashTable(data_t *SlaveAXI, data_t *Master2Mem, data_t *Master2SysAlloc, data_t in_size, data_t in_case){
	...
	hdTable_old = hdTable;
	hdTable = PM_1_INSERTION(Master2Mem, Master2SysAlloc, log2size, buffPtr);
	...
	hdTable_old = hdTable;
	hdTable = PM_2_CHECK_INSERTION(Master2Mem, Master2SysAlloc, hdTable, log2size, buffPtr);
	if(hdTable != hdTable_old){
	...
	hdTable_old = hdTable;
	hdTable = PM_3_UPDATE(Master2Mem, Master2SysAlloc, hdTable, log2size, buffPtr);
	if(hdTable != hdTable_old){
