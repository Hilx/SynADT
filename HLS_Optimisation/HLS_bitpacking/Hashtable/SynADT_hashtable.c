...
void CheckInsert_HashTable(data_t *Master2Mem, data_t *Master2SysAlloc, ptr_t hdTable, int key, int *buffPtr){
	ptr_t search_result = SearchHashTable(Master2Mem, hdTable, key, buffPtr);
...
void UpdateNode_HashTable(data_t *Master2Mem, data_t *Master2SysAlloc, ptr_t hdTable, int oldKey, int newKey, int *buffPtr){
	DeleteNodeHashTable(Master2Mem, Master2SysAlloc, hdTable, oldKey, buffPtr);
	InsertNodeHashTable(Master2Mem, Master2SysAlloc, hdTable, newKey, buffPtr);
}
...
void InsertNodeHashTable(data_t *Master2Mem, data_t *Master2SysAlloc,ptr_t hdTable, int key, int *buffPtr){
	...
	hdList = Add2SortedList(myHeap, Master2SysAlloc, key, hdList, buffPtr);
...
void DeleteNodeHashTable(data_t *Master2Mem, data_t *Master2SysAlloc, ptr_t hdTable, int key, int *buffPtr){
	...
	ptr_t hdList = ReadHashTable(myHeap, hdTable, entryIndex);	
	// read from hash table
	hdList = DeleteListNode_HT(myHeap, Master2SysAlloc, hdList, key, buffPtr);
	// write/update hash table entry pointer value
	WriteHashTable(myHeap, hdTable, entryIndex, hdList);	
}
...
ptr_t SearchHashTable(data_t *Master2Mem, ptr_t hdTable, int key, int *buffPtr){
	...
	entry_index_t entryIndex = HashingAlgorithm(key);
	ptr_t hdList = ReadHashTable(myHeap, hdTable, entryIndex);	
	return SearchInList(myHeap, hdList, key, buffPtr);
}
