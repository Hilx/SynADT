...
void list_node_write_std(data_t *Master2Mem, ptr_t nodePtr, struct list_node_t_std currentNode){
	list_node_set_next_pointer(Master2Mem, nodePtr, currentNode.nextPtr);
	list_node_write_data(Master2Mem, nodePtr, currentNode.data);
}
struct list_node_t_std list_node_read_std(data_t *Master2Mem, ptr_t nodePtr){
	struct list_node_t_std currentNode;
	currentNode.nextPtr = list_node_get_next_pointer(Master2Mem, nodePtr);
	currentNode.data = list_node_read_data(Master2Mem, nodePtr);
	return currentNode;
}
/* Whole Node */
